...

{
	...
	add:[
		...
		// Words
		{
			type:"box",
			id:"title",
			text:"preloader_title", x:180, y:60, w:600, h:230, align:"center"
		},
		{
			type:"box",
			id:"button",
			text:"preloader_button", x:180, y:370, w:600, h:100, align:"center"
		},
		{
			type:"box",
			id:"button",
			text:"skip_tutorial_button", x:180, y:450, w:600, h:100, align:"center"
		}
	...
	onstart: function(slideshow){
		...
		var skipButton = slideshow.boxes.boxes[2].children[0];
	...
	onupdate: function(slideshow, state){
		...
		if(window.PRELOAD_PROGRESS==1 && Simulations.checkServerConnected){
			console.log("wtf");
			state.FULLY_LOADED = true;
			label = getWords("preloader_play");
			skipLabel = getWords("skip_tutorial");
			button.removeAttribute("disabled");
			skipButton.removeAttribute("disabled");

		}
		else if(window.PRELOAD_PROGRESS==1 && !Simulations.checkServerConnected){
			label = "Waiting on Server...";
			skipLabel = "Happens when no players!";
		}
		else{
			label = getWords("preloader_loading") + " ";
			label += Math.round(window.PRELOAD_PROGRESS*100) + "%";
			skipLabel = "this won't take long..."

		}
		button.innerHTML = label;
		skipButton.innerHTML = skipLabel;
