...
subscribe('reference/show', function (ref_id) {
	...
	var footnote = document
		.querySelector('reference#' + ref_id + ' > div')
		.innerHTML.trim();
	...
	showAll: function (thing) {
		...
		$all(thing)
			...
			.forEach(function (thing) {
				var thingHTML = thing.innerHTML;
				// we need to remove endgame_content id so that the original can still be referenced when needing to update content
				thingHTML = thingHTML.replace(
					'endgame_content',
					'endgame_content_modal',
				);
				html += '<div>' + thingHTML + '</div>';
			});
		...
		Modal.show(true);
		if (thing == 'endgame') {
			Simulations.Chart = buildChart(
				Simulations.ScoreLists[0],
				Simulations.ScoreLists[1],
			);
		}
	},
...
function buildChart(playerScore, enemyScore) {
	...
	const colors = {
		// hex codes for various colours if you want to change without much hassle
		green: {
			fill: '#e0eadf',
			stroke: '#5eb84d',
		},
