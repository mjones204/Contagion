function SimUI(container, color){
	...
	startButton.onclick = function(event){
		if(event.ctrlKey){
			...
			if(Simulations.TutorialMode){
				...
				setTimeout((startButton) => {startButton.innerHTML = getWords("sim_start"); self.container.removeAttribute("active");},500, startButton);
			}
			else if (SimUI.RoundNumber < 11){
				if (Simulations.validateMoves(false, -1) && !Simulations.inProgress){
					timerDisplay.innerHTML = getWords("timer_caption_2")+" "+SimUI.timeRemaining;
	...
	var _roundEnd = function(){
			...
			if(!Simulations.TutorialMode){
				...
				if (SimUI.RoundNumber < 10){
					...
					roundDisplay.innerHTML = getWords("round_caption")+" "+"<b>"+SimUI.RoundNumber+"</b>";
					roundDisplay.style.fontSize = "50px";
					percentageInfectedDisplay.innerHTML = "Percentage Infected: " + Simulations.PERCENTAGE_INFECTED+"%";
	...
	SimUI.timerTick = function(){
		SimUI.timeRemaining--;
		var text = Simulations.awaitingResponse ? getWords("timer_caption_2") : getWords("timer_caption_1");
		try{
			...
			SimUI.timerTimeout = setTimeout((text) => {SimUI.timerTick(text);},1000, text);
		}
		catch(err){
			setTimeout((text) => {SimUI.timerTick(text);},1000, text);
		}
	}
