function Peep(config) {
	...
	self.update = function () {
		...
		self.faceX =
			self.faceX * _faceFollow + faceVector.x * (1 - _faceFollow);
		self.faceY =
			self.faceY * _faceFollow + faceVector.y * (1 - _faceFollow);
		...
		if (self.sim.options.splash) {
			...
			friends.forEach(function (friend) {
				...
				var force = multiplyVector(
					getUnitVector(fromTo),
					forceMagnitude,
				);
	...
	self.draw = function (ctx) {
		...
		self.sprite.gotoFrame(myFrame); // responsible for red circle overlay
		self.sprite.draw(ctx);
		...
		if (self.sim.config.x == 0) {
			//HACK: (Updated Version) Can't seem to pass new variables for Sim, so I'm using the existing x value, which is always 0 for gameplay, and something else for the intro.
			...
			if (self.numFriends > 0) {
				const probGreenInfect =
					self.numInfectedFriends / self.numFriends;

				if (Simulations.DrawPieCharts) {
					self.sprite.drawPieChart(
						1 - probGreenInfect,
						probGreenInfect,
					);
				}
				...
				// spinners
				if (Simulations.pieSpinnersActive) {
					const greenWins = Simulations.p1OwnsNode(self.id);
					self.sprite.startPieSpinner(greenWins);
				} else {
					self.sprite.stopDrawPieSpinner();
				}
				// console.log(
				// 	'nodeID',
				// 	self.id,
				...
				if (Simulations.DebugMode) {
					var labelPercent =
						Math.round(100 * probGreenInfect) + '% ' + this.id;
				} else {
					var labelPercent = Math.round(100 * probGreenInfect) + '%';
				}
			...
			// for the random selector bar
			// in the perform infections in the server we assign the chosen number to the peep
			// when we receive this info before we fire the next round we have the bars move for 3 sec
			// they land in the green/red
		}
