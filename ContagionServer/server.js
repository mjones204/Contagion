...
Server.initialiseTopologyLayoutIndexes = function () {
	...
	for (var i = 0; i < serverConfigs.length; i++) {
		//If 2 topologies, will be length 2 (layouts do not count)
...
function Server() {
	...
	Server.AiStrategy = 'SimpleGreedy'; // See 'aiTurn' method for each string
	...
	// For overriding Database Values - Very rarely needed!
	Server.OverrideDBValues = false;
	Server.OverrideDB_gameId = 'TEST-GAME-RANDOM-VS-EQUILIBRIUM';
	Server.OverrideDB_p1Id = 'TestRandom';
}
...
class GameState {
	constructor(
		...
		this.timer = setInterval(
			...
			Server.heartbeatCheckFrequency,
			this,
		); //Checks heartbeats
...
GameState.prototype.addGameToDatabase = function () {
	...
	var p1_str = this.playerOneLayoutID
		.replace('Topology_newer', '')
		.split('_');
	var p2_str = this.playerTwoLayoutID
		.replace('Topology_newer', '')
		.split('_');
	...
	let _p1id = p1id;
	// very rarely used
	if (Server.OverrideDBValues) {
		_gameID = Server.OverrideDB_gameId;
		_p1id = Server.OverrideDB_p1Id;
	}

	var query = `INSERT INTO master_games_table VALUES ('${_gameID}', '${timestamp}', '${_p1id}', '${p2id}', '${infectedPeepsString}', '${Server.RemoveOldNodes}', ${p1_str[0]}, ${p1_str[1]}, ${p2_str[0]}, ${p2_str[1]});`;
...
GameState.prototype.updateGameDatabaseEntry = function () {
	...
	var query = `UPDATE master_games_table SET player_one_id = '${_p1id}', player_two_id = '${p2id}' WHERE game_id = '${_gameID}';`;
	Server.sendSqlQueryGame(query, this);
};
...
GameState.prototype.addMovesToDatabase = function () {
	...
	var query = `INSERT INTO player_actions_table VALUES ('${_gameID}', ${this.roundNumber}, '${this.flippedNodes}', ${p1_last_move} , ${p2_last_move}, ${this.playerOneTime}, ${this.playerTwoTime}, '${p1Nodes}', '${p2Nodes}');`;
	Server.sendSqlQueryGame(query, this);
...
GameState.prototype.addPlayerMoves = function (
	moves,
	isPlayerOne,
	opponentReady,
) {
	...
	if (opponentReady) {
		...
		if (
			Server.AiWaiting == true &&
			(this.playerTwo == 'AI' || this.playerOne == 'AI')
		) {
			this.fakeAiWait();
		} else {
		...
		Server.sendClientMessage(
			new Message(null, 'DEFERRED_STATE_TOKEN'),
			recipient,
		);
	}
};
...
GameState.prototype.addPlayerOneMoves = function (moves) {
	...
	this.addPlayerMoves(
		moves,
		true,
		this.playerTwo == 'AI' || this.playerTwoMoves.length > 0,
	);
...
GameState.prototype.addPlayerOneTimer = function (duration) {
	...
	Server.sendClientMessage(
		new Message([1, duration - 1], 'TIMER_TOKEN'),
		game.playerOne,
	);
};
...
GameState.prototype.addPlayerTwoMoves = function (moves) {
	...
	this.playerTwoTime =
		Date.now() - this.gameStartTime - this.playerTwoTimeOffset;
	this.addPlayerMoves(
		moves,
		false,
		this.playerOne == 'AI' || this.playerOneMoves.length > 0,
	);
};
...
GameState.prototype.heartbeatHandler = function (game) {
	...
	if (!Server.demoMode && !Server.ExperimentMode) {
		...
		if (
			game.playerTwo !== 'AI' &&
			game.playerTwo !== null &&
			now - game.playerTwoLastHeartbeat > Server.heartAttackTime
		) {
...
GameState.prototype.registerClick = function (playerID, nodeID, action) {
	...
	var query = `INSERT INTO player_clicks_table VALUES ('${_gameID}', ${playerID}, '${nodeID}', '${action}', '${timestamp}', '${this.roundNumber}');`;
	Server.sendSqlQueryGame(query, this);
};
...
GameState.prototype.recordMoveNodeDegrees = function () {
	...
	// add node degree of player's last moves to their respective arrays
	const p1LastMove = this.playerOneMoves[this.playerOneMoves.length - 1];
	this.playerOneNodeDegreeList.push(nodeDegrees[p1LastMove]);
	const p2LastMove = this.playerTwoMoves[this.playerTwoMoves.length - 1];
	...
	updatedPeeps.forEach(function (peep, index) {
		...
		if (peep[4] > 0) {
			...
			if (Server.InfectionMode == 'majority') {
				//For the majority model (i.e. only change colour if one side has more sources of influence)
...
GameState.prototype.toState = function () {
	// create cloned state object from current game state

	const state = {
		gameID: this.gameID,
		rngThreshCount: this.rngThreshCount,
		roundNumber: this.roundNumber,
		p1Moves: this.playerOneMoves.slice(0),
		...
		roundLimit: Server.RoundLimit,
		lastRoundBonus: Server.LastRoundBonus,
		gameOver: false,
		winner: -1,
	};
...
GameState.prototype.aiTurn = function (aiMoves, friendlyNodeStatus, strategy) {
	...
	switch (strategy) {
		...
		case 'MonteCarlo':
			...
			break;
		case 'SimpleGreedy':
			aiTurnSimpleGreedy = require('./MyopicGreedy'); //Calls the external file
		...
		case 'GreedyPredictsHigh':
			aiTurnSimpleGreedy = require('./MyopicGreedy');
			ctx = this;
		...
		case 'GreedyPredictsGreedy':
			...
			aiTurnSimpleGreedy(
				...
				friendlyNodeStatus,
				'Greedy',
			); //don't need to remove worst token, so just false
			break;
		case 'Equilibrium':
			this.aiTurnEquilibrium(aiMoves, oneNodeOnly, friendlyNodeStatus);
			break;
		case 'DSLow':
			this.aiTurnDegreeSensitive(
				aiMoves,
				oneNodeOnly,
				true,
				friendlyNodeStatus,
			); //True for low degree preference
			break;
		case 'DSHigh':
			this.aiTurnDegreeSensitive(
				aiMoves,
...
//random strategy
GameState.prototype.aiTurnRandom = function (
	aiMoves,
	oneNodeOnly,
	friendlyNodeStatus,
) {
...
GameState.prototype.aiTurnEquilibrium = function (
	aiMoves,
	oneNodeOnly,
	friendlyNodeStatus,
) {
	...
	if (Server.TokenProtocol == 'Incremental') {
		...
		if (this.isServerPlayer(friendlyNodeStatus)) {
			//Determines which moves are the current players'
		...
		for (i = 0; i < Server.NumberOfNodes; i++) {
			...
			var probabilitiesVector = this.createProbabilitiesVector(
				laplacian,
				friendlyMovesVector,
				i,
			);
		...
		this.createProbabilitiesVector(
			laplacian,
			friendlyMovesVector,
			bestNode,
		);
		...
		console.error(
			"ERROR! This algorithm hasn't been developed for non-incremental token protocol yet!",
		);
	}
};

//Creates vector of probabilities that each node will be owned by the agent running this code.
GameState.prototype.createProbabilitiesVector = function (
	laplacian,
	friendlyMovesVector,
	i,
) {
...
GameState.prototype.aiTurnDegreeSensitiveTest = function (
	aiMoves,
	oneNodeOnly,
	lowDegreeSensitivity,
	friendlyNodeStatus,
) {
	if (true) {
		return;
...
GameState.prototype.aiTurnDegreeSensitive = function (
	aiMoves,
	oneNodeOnly,
	lowDegreeSensitivity,
	friendlyNodeStatus,
	...
	if (Server.TokenProtocol == 'Incremental') {
		...
		console.error(
			"ERROR! This algorithm hasn't been developed for non-incremental protocol yet!",
		);
	}
};
...
GameState.prototype.aiTurnMirror = function (
	aiMoves,
	oneNodeOnly,
	friendlyNodeStatus,
) {
...
Server.submitMoves = function (message, ws) {
	...
	//Validates no. tokens
	if (
		Server.TokenProtocol == 'Incremental' &&
		message.length != game.roundNumber + 1
	) {
		console.error(
...
Server.getConfig = function (twoPlayerMode, perm) {
	...
	if (perm == undefined) {
		...
		Server.CurrentTopologyIndex =
			(Server.CurrentTopologyIndex + 1) % serverConfigs.length;
		//P1 Topology
		...
		Server.CurrentTopologyLayoutIndexes[topologyID] =
			(Server.CurrentTopologyLayoutIndexes[topologyID] + 1) %
			serverConfigs[topologyID].length;
...
Server.newGame = function (username, ws) {
	...
	if (Server.AiMode || Server.WaitingGameConfig == null) {
		...
		game = new GameState(
			config.network.peeps,
			config.network.connections,
			config.playerOneLayoutID,
			config.playerTwoLayoutID,
			config.laplacianID,
			ws,
		);
		...
		config.maxConnections =
			Server.TokenProtocol == 'Incremental' ? 1 : Server.MAX_TOKENS;
		...
		if (playerNo == 1) {
			Server.sendClientMessage(
				new Message(
					[
						result,
						game.playerOneScoreList,
						game.playerTwoScoreList,
						game.gameID,
						playerNo,
					],
					'GAME_END_TOKEN',
...
Server.startTimer = function (game, status, duration, isPlayerOne) {
	...
	if (isPlayerOne) {
		if (!game.playerOne.id.startsWith('Exp_AI_')) {
			//doesn't set timers for the experiments. They shouldn't be an issue, but just in case!
			game.playerOneTimer = setTimeout(
				(isPlayerOne) => {
		...
		Server.sendClientMessage(
			new Message([0, duration - 1], 'TIMER_TOKEN'),
			game.playerOne,
		);
	} else {
		game.playerTwoTimer = setTimeout(
			(isPlayerOne) => {
				game.outOfTime(isPlayerOne);
			},
			duration * 1000,
...
Server.getAllGamesPlayedByUser = async function (userID) {
	var query =
		"SELECT * FROM master_games_table WHERE player_one_id = '" +
		userID +
		"' OR player_two_id = '" +
		userID +
		"'";
...
Server.getAllRoundsInfoForGame = async function (gameID) {
	var query =
		"SELECT * FROM player_actions_table WHERE game_id = '" +
		gameID +
...
Server.sendMTurkInfoToClient = async function (payload, ws) {
	...
	var mturkInfo = await Server.getMTurkInfoForClient(
		username,
		lastGameID,
		ws,
	);
	...
	for (var i = 0; i < games.length; i++) {
		...
		if (rounds.length == 10) {
			...
			for (var r = 0; r < rounds.length; r++) {
				...
				// calculate scores
				ourScore = Server.calculatePlayerScoreFromNodes(
					ourScore,
					ourNodes,
					round.round_number,
				);
				opponentScore = Server.calculatePlayerScoreFromNodes(
					opponentScore,
					opponentNodes,
					round.round_number,
				);
			...
			var gameReward = Server.calculateMTurkRewardForGame(
				ourScore,
				winner,
			);
...
Server.calculateMTurkRewardForGame = function (score, win) {
	...
	var MTURK_MAX_SCORE_REWARD = 0.2;
...
Server.sendCompletionCodeToClient = async function (username, ws) {
	...
	if (completion_code) {
		Server.sendClientMessage(
			new Message(completion_code, 'COMPLETION_CODE'),
			ws,
		);
	} else {
...
Server.getCompletionCodeForPlayer = async function (username) {
	...
	var query =
		"SELECT * FROM mturk_completion_table WHERE player_id = '" +
		username +
		"'";
	var result = await Server.sendSqlQuery(query);
...
Server.getNextMoveForPlayerFromStrategy = async function (
	game,
	playerNumber,
	strategy,
) {
	...
	if (strategy == 'Manual') {
		...
		(strategy == 'Mirror' || strategy == 'Equilibrium') &&
		game.roundNumber == 0
	) {
		return 'random';
	}
...
Server.sendTestPlayerNextMoveInfoToClient = async function (payload, ws) {
	...
	if (game == null) {
		Server.sendClientMessage(
			new Message(info, 'TEST_PLAYER_NEXT_MOVE'),
			ws,
		);
		return;
	}
	...
	info.nextMove = await Server.getNextMoveForPlayerFromStrategy(
		game,
...
Server.ParseMessage = function (message, ws) {
	...
	switch (
		message.status //If you want to send/recieve new types of message between client/server, you can handle the delivery here
	) {
